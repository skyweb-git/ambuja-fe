...
import { useWebsiteContent, DEFAULT_CONTENT } from '../services/contentService';

export default function OurProjects({ onOpenProjectInquiry }) {
  const content = useWebsiteContent();
  const projectsSection = content?.projectsSection || DEFAULT_CONTENT.projectsSection;
    ...
    <section id="projects" className="section-wrapper projects-section" aria-label="Our Projects and Townships">
      <div className="section-container">
        ...
        <div className="section-header text-center">
          <span className="eyebrow-tag">{projectsSection?.eyebrowTag || 'LANDMARK DEVELOPMENTS'}</span>
          <h2 className="section-title">
            {projectsSection?.title || 'Our Projects'}
          </h2>
          <p className="section-subtitle">
            {projectsSection?.subtitle || 'Explore premier master-planned townships and signature villa communities developed with unmatched luxury, architectural brilliance, and strategic connectivity.'}
          </p>
        </div>
          ...
          {projects.map((project, idx) => (
            <div key={project.id || idx} className="project-card">
              <div className="project-card-image-wrap">
                <img
                  src={project.image || CLOUDINARY_MEDIA.gallery[0]?.url || 'https://res.cloudinary.com/s8b4ps7b/image/upload/v1788847939/maytri_ambhuja/gallery/gallery_001.jpg'}
